...
void DlgTrackInfo::saveTrack() {
    ...
    qDebug() << "=== DlgTrackInfo::saveTrack() DEBUG ===";
    qDebug() << "TrackRecord genre string:"
             << m_trackRecord.getMetadata().getTrackInfo().getGenre();
    qDebug() << "Widget genres:" << (m_pGenreWidget ? m_pGenreWidget->getGenres() : QStringList());

    if (m_pTrackCollection) {
        QStringList dbGenresBefore =
                m_pTrackCollection->getGenreDao().getGenresForTrack(
                        m_pLoadedTrack->getId());
        qDebug() << "Database genres BEFORE replaceRecord:" << dbGenresBefore;
    }
    ...
    if (m_pTrackCollection) {
        QStringList dbGenresAfter =
                m_pTrackCollection->getGenreDao().getGenresForTrack(
                        m_pLoadedTrack->getId());
        qDebug() << "Database genres AFTER replaceRecord:" << dbGenresAfter;
    }
}
